using EASSET.Administration;

namespace EASSET.Membership.Pages;
        ...
        IEmailSender emailSender, ISMSService smsService)
    {
             ...
             (user.TwoFactorAuth == TwoFactorAuthType.Email)))
        {
            ...
            if (user.TwoFactorAuth == TwoFactorAuthType.SMS)
            {
                ...
                smsService.Send(
                    phoneNumber: mobile,
                    text: "Please use code " + data.TwoFactorCode + " for EASSET login.",
                    reason: "Sent by EASSET system for two factor authenication by SMS (" + user.Username + ")");
            ...
            else
            {
                ...
                emailSender.Send(
                    mailTo: user.Email,
                    subject: "Your two-factor authentication code for EASSET login",
                    body: "Please use code " + data.TwoFactorCode + " for EASSET login.");
